...
const TestDetails = () => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            ...
            const response = await axios.get(`http://localhost:4000/api/v1/admin/routes/tests/${testId}/get-questions`, {
        ...
        const fetchResponses = async () => {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://localhost:4000/api/v1/admin/routes/tests/${testId}/get-responses`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (response.status === 200) {
